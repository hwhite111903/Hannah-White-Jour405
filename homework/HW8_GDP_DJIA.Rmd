...
```{r}
GDP_data |> summarise(correlation = cor(DJIA, GDP, method = "pearson"))
```
...
Both the correlation calculation and the scatterplot show a positive relationship. With the calculation, the number given was above zero and close to 1, so there is a decently strong positive relationship. For the scatterplot, you can see a steady incline and a tight line aswell.
