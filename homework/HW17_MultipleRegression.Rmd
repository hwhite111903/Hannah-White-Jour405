...
Voter turnout seems to go up in counties where more people are white, older, and earn more money. It goes down in places with higher unemployment. Education and population size don’t seem to have a strong link to turnout. Some of the independent variables are also closely related to each other—like income and education, or population and percent white—which could cause problems in a model. 

Scatterplots show clear patterns between turnout and things like unemployment and percent white. The data shows that turnout ranges from 58% to 80%, and counties are very different in terms of income and racial makeup.
...
This model does a good job predicting voter turnout. It explains about 85% of the differences in turnout, and the fit stays strong even after adjusting for the number of variables. Income, median age, and the percentage of white residents are the most important predictors. Higher income and older age are linked to higher turnout, and counties with more white residents also tend to have higher turnout.

Some factors didn’t matter as much. Population size, education level, and unemployment were not statistically significant. That means they didn’t show a strong or clear link to turnout in this model.
...
Models 1 and 2 have the highest adjusted R-squared (0.8023), meaning they explain turnout best. Model 3 performs slightly worse (0.7362), while Model 4, with only population and income, performs significantly worse (0.4850). Overall, Model 1 or 2 is preferred, with Model 4 clearly the weakest.

```{r}
# Model 2
model2 <- lm(turnout ~ population + income + pct_bachelors_higher + pct_unemployed + median_age + pct_white, data = md_turnout)
summary(model2)

# Model 3
model3 <- lm(turnout ~  population + income + pct_unemployed + median_age, data = md_turnout)
summary(model3)
...
This data shows that voter turnout is higher in places where people have higher incomes, are older, and where more residents are white. In contrast, population size, education levels, and unemployment don’t seem to make much difference in whether people vote. 

This data could support an election story by spotlighting which demographic and economic factors most influence voting. For example, we could focus on how older, more affluent, and less diverse communities are more likely to vote, raising questions about equity in political representation and where outreach efforts should be focused.
